...
def overlap(a, b):
    return not (a["seg_end"] <= b["seg_start"] or b["seg_end"] <= a["seg_start"])
...
def is_same_program(a, b):
    return a["program_id"] == b["program_id"] and a["channel_id"] == b["channel_id"]
...
class CandidatePool:
    def __init__(self, segments):
        ...
        self.all_segments.sort(key=lambda x: (x["seg_start"], x["seg_end"], x["channel_id"], x["program_id"]))
    ...
    def alternatives_for_segment(self, seg, slack=45, limit=80):
        ...
        cands.sort(key=lambda x: (-seg_value(x), abs(x["seg_start"] - seg["seg_start"]), abs(x["seg_end"] - seg["seg_end"])))
        return cands[:limit]
...
class Evaluator:
    ...
    def recompute_segment_fields(self, s):
        ...
        s["bonus"] = compute_bonus(s["seg_start"], s["seg_end"], s["genre"], self.D, self.time_prefs)
        return s
...
class RepairEngine:
    ...
    def _greedy_select(self, candidates):
        ordered = sorted(
            ...
            key=lambda x: (x["seg_start"], x["seg_end"], -seg_value(x), x["channel_id"], x["program_id"])
        )
    ...
    def _drop_conflicts_by_value(self, schedule):
        changed = True
        sched = deep_copy_schedule(schedule)
        ...
        while changed:
            ...
            while i < len(sched):
                a = sched[i - 1]
                b = sched[i]
                if a["seg_start"] < b["seg_end"] and b["seg_start"] < a["seg_end"]:
                    va = seg_value(a)
                    vb = seg_value(b)
                    if va < vb:
                        sched.pop(i - 1)
                        ...
                        # Tie-break: keep earlier ending one
    ...
    def _fill_gaps(self, schedule, rounds=2):
        ...
        for _ in range(rounds):
            ...
            for gs, ge in gaps:
                ...
                cands = self.pool.gap_candidates(gs, ge, limit=100)
                cands.sort(key=lambda x: (-seg_value(x), x["seg_start"], x["seg_end"]))
                ...
                for cand in cands:
                    ...
                    ev = self.ev.evaluate(trial)
                    cur_ev = self.ev.evaluate(sched)
                    if ev["feasible"] and ev["score"] > cur_ev["score"]:
                        sched = trial
                        inserted_any = True
                        break
    ...
    def repair(self, schedule):
        ...
        # normalize and keep only basically valid segments
        sched = [self.ev.recompute_segment_fields(dict(s)) for s in schedule]
        sched = [s for s in sched if self.ev.segment_valid_basic(s)]
        ...
        # conflict removal, compatibility filtering, genre run fix, then gap filling
        ...
        # fallback: greedy from scratch on current content
...
class GeneticOperators:
    ...
    def crossover_block_mix(self, p1, p2):
        ...
        t2_choices = [t for t in times if t > t1]
        t2 = self.rng.choice(t2_choices)
    ...
    def mutate_replace_segment(self, schedule):
        ...
        child = deep_copy_schedule(schedule)
        idx = self.rng.randrange(len(child))
        target = child[idx]
        alternatives = self.pool.alternatives_for_segment(target, slack=60, limit=60)
    ...
    def mutate_insert_gap(self, schedule):
        ...
        cands = self.pool.gap_candidates(gs, ge, limit=80)
        if not cands:
            return child

        top = cands[: min(10, len(cands))]
        cand = self.rng.choice(top)
    ...
    def mutate_local_shuffle(self, schedule):
        if len(schedule) < 2:
            return deep_copy_schedule(schedule)
...
class HybridGAScheduler:
    ...
    def score(self, schedule):
        ...
        if key not in self.cache:
            self.cache[key] = self.ev.evaluate(schedule)
    ...
    def initialize_population(self):
        ...
        # 1) exact deterministic elite
        pop.append(deep_copy_schedule(self.deterministic_seed))

        # 2) lightly mutated seed copies
        ...
        while len(pop) < self.population_size:
            ...
            if self.rng.random() < 0.35:
                # inject a few random nearby candidates
                inject_n = self.rng.randint(1, 3)
                for _ in range(inject_n):
                    cand = random_choice_or_none(self.pool.all_segments[: min(120, len(self.pool.all_segments))], self.rng)
                    if cand is not None:
                        child.append(dict(cand))
                child = self.repair_engine.repair(child)
            pop.append(child)

        # unique + sorted by score
        unique = {}
        for ind in pop:
            unique[schedule_key(ind)] = ind
        ...
        while time.time() - start < time_limit_seconds:
            ...
            while len(new_population) < self.population_size and time.time() - start < time_limit_seconds:
                ...
                if self.rng.random() < self.crossover_rate:
                    ...
                    child = deep_copy_schedule(p1 if self.score(p1)["score"] >= self.score(p2)["score"] else p2)
            ...
            # mild diversification if stagnant
...
def run_one_ga(data, top_k=8, population_size=24, elite_count=2, time_limit=300, seed=42):
...
def run_multi(data, runs=10, time_limit=300, top_k=8, population_size=24, elite_count=2, base_seed=42):
    all_runs = []
    best_result = None
    best_score = NEG_INF
...
def main():
    ...
    parser.add_argument("--top-k", type=int, default=8, help="Top K segments kept per program from deterministic preprocessing")
